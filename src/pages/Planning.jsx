...
const Planning = () => {
    // -------------------------------------------------------------
    // FIX MAJEUR: On n'utilise PLUS le scroll "whileInView" (qui 
    // buggait et laissait la page bleue vide selon le navigateur).
    // On force l'animation magique AU LANCEMENT DE LA PAGE !
    // -------------------------------------------------------------

    // Effet WOW Ultra-Bourrin et sécurisé pour débloquer la page :
    const wowParent = {
        hidden: { opacity: 1 }, // Garantit qu'on n'est jamais invisible !!
        visible: {
            opacity: 1,
            transition: { staggerChildren: 0.15, delayChildren: 0.1 } // Rapide
        }
    };
    ...
    const wowLeftItems = {
        ...
        visible: { 
            ...
            x: 0, 
            scale: 1,
            transition: { type: "spring", stiffness: 120, damping: 10 } // Gros rebond Wow
        }
    };
    ...
    const wowRightItems = {
        ...
        visible: { 
            ...
            rotateY: 0,
            scale: 1,
            transition: { type: "spring", stiffness: 100, damping: 12 } // 3D Tilt + Rebond
        }
    };
    ...
    return (
        // IMPORTANT: No hidden overflow wrappers here blocking visibility
        <div className="bg-[#071341] py-20 min-h-screen" style={{ perspective: "1500px" }}>
            ...
            >
                ...
                {/* ---------- GAUCHE (Entrée explosive depuis la gauche) ---------- */}
                <div className="py-6 md:col-span-5">
                    <motion.p variants={wowLeftItems} className="mb-2 text-[#B8AB38]">
                        CÉRÉMONIE 2025
                    </motion.p>
                    
                    <motion.h1 variants={wowLeftItems} className="mb-2 md:text-5xl text-3xl font-bold">
                        PLANNING <br />DE LA SOIRÉE
                    </motion.h1>
                    
                    <motion.div variants={lineDraw} className="w-36 h-[3px] bg-white mt-3 origin-left"></motion.div>
                    
                    <motion.p variants={wowLeftItems} className="mt-5 md:w-[80%] text-justify font-poppins uppercase">
                    ...
                    <motion.div variants={wowLeftItems} className='mt-7'>
                        <a 
                ...
                {/* ---------- DROITE (Entrée explosive 3D depuis la droite) ---------- */}
                <div className="py-5 md:py-16 md:col-span-7">
                    <div className="space-y-8">
                        {listItems.map((item, index) => (
                            <motion.div key={index} variants={wowRightItems} className="group">
